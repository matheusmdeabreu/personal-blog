...
def admin_only(func):
    ...
    def decorated_function(*args, **kwargs):
        # Verifica se o usuário está autenticado e se é o administrador (id == 1)
        if not current_user.is_authenticated or current_user.id != 1:
            abort(403)  # Retorna erro 403 (Forbidden) se não autorizado
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if existing_user:
            flash(f"You've already signed up with that email. Please log in instead.", 'flash')
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            if check_password_hash(user.password, password):
                ...
                flash('Login unsuccessful. Please check your password and try again.', 'flash')
        else:
            flash('Login unsuccessful. Email not found. Please try again or register.', 'flash')
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            ...
            flash('You must be logged in to comment.', 'flash')
            return redirect(url_for('login'))
